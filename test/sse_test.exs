defmodule ArchAstro.SDK.SSETest do
  use ExUnit.Case, async: true

  test "parses events framed with bare carriage returns" do
    assert {[%ArchAstro.SDK.SSE.Event{event: "message", data: %{"ok" => true}}], ""} =
             ArchAstro.SDK.SSE.parse_events("event: message\rdata: {\"ok\":true}\r\r")
  end

  test "preserves a CRLF separator split across chunks" do
    assert {[], buffer} = ArchAstro.SDK.SSE.parse_events("data: {\"ok\":true}\r")

    assert {[%ArchAstro.SDK.SSE.Event{data: %{"ok" => true}}], ""} =
             ArchAstro.SDK.SSE.parse_events(buffer <> "\n\r\n")
  end
  ...
  test "stream enumeration leaves unrelated caller messages in the mailbox" do
    ...
    assert {:ok, client} = ArchAstro.SDK.Client.for_server(server, req: Req.new(plug: plug))
    send(self(), :unrelated)

    assert [%ArchAstro.SDK.SSE.Event{data: %{"ok" => true}}] =
             ArchAstro.SDK.SSE.stream(client, :get, "/events") |> Enum.to_list()

    assert_received :unrelated
  end

  test "removes only one optional space after an SSE field colon" do
    assert {[%ArchAstro.SDK.SSE.Event{event: "  note", data: "  hello"}], ""} =
             ArchAstro.SDK.SSE.parse_events("event:   note\ndata:   hello\n\n")
  end

  test "defaults unnamed SSE events to message" do
    assert {[%ArchAstro.SDK.SSE.Event{event: "message", data: %{"ok" => true}}], ""} =
             ArchAstro.SDK.SSE.parse_events("data: {\"ok\":true}\n\n")

    assert {[%ArchAstro.SDK.SSE.Event{event: "message", data: %{"ok" => true}}], ""} =
             ArchAstro.SDK.SSE.parse_events("event:\ndata: {\"ok\":true}\n\n")
  end
  ...
  test "replaces malformed UTF-8 without suppressing later events" do
    ...
    assert {[%ArchAstro.SDK.SSE.Event{data: %{"ok" => true}}], ""} =
             ArchAstro.SDK.SSE.parse_events(input)
  end

  test "implements colonless, id, and retry field semantics" do
    input = "data\nid: keep\nid: bad#{<<0>>}value\nretry: invalid\nretry: 1500\n\n"

    assert {[%ArchAstro.SDK.SSE.Event{data: "", id: "keep", retry: 1500}], ""} =
             ArchAstro.SDK.SSE.parse_events(input)
  end

  test "ignores exactly one UTF-8 BOM at the start of a stream" do
    assert {[%ArchAstro.SDK.SSE.Event{data: "first"}], ""} =
             ArchAstro.SDK.SSE.parse_events(<<0xEF, 0xBB, 0xBF>> <> "data: first\n\n")

    assert {[], _buffer} =
             ArchAstro.SDK.SSE.parse_events(<<0xEF, 0xBB, 0xBF>> <> "data: later\n\n", false)
  end

  test "buffers a UTF-8 BOM split across transport chunks" do
    assert {[], first} = ArchAstro.SDK.SSE.parse_events(<<0xEF>>)

    assert {[%ArchAstro.SDK.SSE.Event{data: "first"}], ""} =
             ArchAstro.SDK.SSE.parse_events(first <> <<0xBB, 0xBF>> <> "data: first\n\n")
  end
  ...
  test "rejects successful responses that are not event streams" do
    ...
    assert_raise ArchAstro.SDK.Error, fn ->
      ArchAstro.SDK.SSE.stream(client, :get, "/events") |> Enum.to_list()
    end
  end
