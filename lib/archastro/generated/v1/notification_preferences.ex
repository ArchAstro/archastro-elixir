# Copyright (c) 2026 ArchAstro Inc. Licensed under the MIT License.
# This file is auto-generated by @archastro/sdk-generator. Do not edit.
# Content hash: 2ec76543bf89

defmodule ArchAstro.SDK.Types.Operations.PutApiV1NotificationPreferences.Input do
  @moduledoc """
  Create or update a notification preference
  """
  @enforce_keys [:channel, :enabled, :type]
  defstruct app_id: :__archastro_unset__, channel: nil, enabled: nil, type: nil

  @type t :: %__MODULE__{
          app_id: String.t() | ArchAstro.SDK.Unset.t(),
          channel: String.t(),
          enabled: boolean(),
          type: String.t()
        }

  @fields [
    app_id: {"app_id", {:optional, :string}},
    channel: {"channel", :string},
    enabled: {"enabled", :boolean},
    type: {"type", :string}
  ]
  @spec from_map(ArchAstro.SDK.JSON.object()) :: t()
  def from_map(data), do: ArchAstro.SDK.Codec.struct_from_map(__MODULE__, data, @fields)
  @spec to_map(t()) :: ArchAstro.SDK.JSON.object()
  def to_map(value), do: ArchAstro.SDK.Codec.struct_to_map(value, @fields)
  @spec matches?(ArchAstro.SDK.JSON.t()) :: boolean()
  def matches?(value), do: ArchAstro.SDK.Codec.matches?(value, {:object, Keyword.values(@fields)})
end

defmodule ArchAstro.SDK.V1.NotificationPreferences do
  @moduledoc """
  NotificationPreference API resource.
  """

  @doc """
  Delete a notification preference

  Removes the authenticated user's explicit notification preference for a
  given `(type, channel)` combination, reverting that slot to the type's
  default channel set.

  The `app_id` param scopes the deletion to a specific app's preference
  row. Omit `app_id` to target the system-level (no-app) slot. Because
  the two slots are stored independently, omitting `app_id` will not
  match a row that has one set, and vice versa.

  Returns `204 No Content` on success. Returns `404` if no preference
  exists for the given composite key.


  Empty response body. A `204 No Content` status indicates the preference was deleted successfully.
  """
  @spec remove(ArchAstro.SDK.Client.t()) :: {:ok, :ok} | {:error, ArchAstro.SDK.Error.reason()}
  def remove(client) do
    path = "/api/v1/notification_preferences"
    ArchAstro.SDK.HTTP.request(client, :delete, path, decode: :void)
  end

  @doc """
  List notification preferences

  Returns all explicit notification preferences belonging to the authenticated
  user. Preferences are returned for every `(type, channel)` combination the
  user has explicitly configured; slots that have not been overridden are not
  included and fall back to the type's defaults.

  The recipient is derived from the authenticated viewer. You cannot retrieve
  preferences for any other user through this endpoint. All configured
  preferences — system-level and app-scoped — are returned together in the
  `data` array.


  An object with a `data` array containing all explicit notification preferences for the authenticated user.
  """
  @spec list(ArchAstro.SDK.Client.t()) ::
          {:ok, ArchAstro.SDK.Types.NotificationPreferenceList.t()}
          | {:error, ArchAstro.SDK.Error.reason()}
  def list(client) do
    path = "/api/v1/notification_preferences"

    ArchAstro.SDK.HTTP.request(client, :get, path,
      decode: {:ref, ArchAstro.SDK.Types.NotificationPreferenceList}
    )
  end

  @doc """
  Create or update a notification preference

  Creates or replaces the authenticated user's notification preference for a
  given `(type, channel)` combination. This is an idempotent PUT: if no
  preference exists for the composite key, a new row is created; if one
  already exists, its `enabled` flag is updated to the value you provide.

  The recipient is derived from the authenticated viewer. You cannot set
  preferences for another user through this endpoint.

  Pass `app_id` to scope the preference to a specific app's notifications —
  most useful for the `app_*` notification type family. Omit `app_id` to
  configure the system-level (no-app) slot. System-level and app-scoped
  preferences are stored independently and do not overwrite each other.

  The `in_app` channel is not configurable and will be rejected with a
  validation error if supplied.


  The created or updated notification preference reflecting the new `enabled` state.
  """
  @spec replace(
          ArchAstro.SDK.Client.t(),
          ArchAstro.SDK.Types.Operations.PutApiV1NotificationPreferences.Input.t()
        ) ::
          {:ok, ArchAstro.SDK.Types.NotificationPreference.t()}
          | {:error, ArchAstro.SDK.Error.reason()}
  def replace(client, input) do
    path = "/api/v1/notification_preferences"

    ArchAstro.SDK.HTTP.request(client, :put, path,
      body: input,
      decode: {:ref, ArchAstro.SDK.Types.NotificationPreference}
    )
  end
end
